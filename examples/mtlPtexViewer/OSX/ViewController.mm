...
-(void)viewDidLoad {
    ...
    [_displacementModePopup removeAllItems];
    [_displacementModePopup addItemsWithTitles:@[@"HW Bilinear", @"Bilinear", @"BiQuadratic", @"None"]];
    [_displacementModePopup selectItemAtIndex:0];
    [_displacementModePopup synchronizeTitleAndSelectedItem];
    
    [_colorModePopup removeAllItems];
    [_colorModePopup addItemsWithTitles:@[@"Ptex Nearest", @"Ptex Bilinear", @"Ptex HW Bilinear", @"Ptex Biquadratic", @"Patch Type", @"Patch Coord", @"Normal", @"None"]];
    [_colorModePopup selectItemAtIndex:0];
    [_colorModePopup synchronizeTitleAndSelectedItem];
    
    [_normalModePopup removeAllItems];
    [_normalModePopup addItemsWithTitles:@[@"HW Screenspace", @"Screenspace", @"BiQuadratic", @"BiQuadratic WG", @"Surface"]];
    [_normalModePopup selectItemAtIndex:0];
    [_normalModePopup synchronizeTitleAndSelectedItem];
...
-(void)_applyOptions {
    ...
    _osdRenderer.displacementMode = (DisplacementMode)self.displacementModePopup.indexOfSelectedItem;
    _osdRenderer.normalMode = (NormalMode)self.normalModePopup.indexOfSelectedItem;
    _osdRenderer.colorMode = (ColorMode)self.colorModePopup.indexOfSelectedItem;
}
...
-(void)popupChanged:(NSPopUpButton*)sender {
    ...
    } else if(sender == self.normalModePopup) {
        _osdRenderer.normalMode = (NormalMode)sender.indexOfSelectedItem;
    } else if(sender == self.displacementModePopup) {
        _osdRenderer.displacementMode = (DisplacementMode)sender.indexOfSelectedItem;
    } else if(sender == self.colorModePopup) {
        _osdRenderer.colorMode = (ColorMode)sender.indexOfSelectedItem;
    }
}

-(void)sliderChanged:(NSSlider *)sender {
    if(sender == _displacementScaleSlider) {
        _osdRenderer.displacementScale = sender.floatValue;
    } else if(sender == _mipmapBiasSlider) {
        _osdRenderer.mipmapBias = sender.floatValue;
    }
}
