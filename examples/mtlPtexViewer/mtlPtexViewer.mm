...
-(void)_renderMesh:(id<MTLRenderCommandEncoder>)renderCommandEnocder {
    ...
    [renderCommandEnocder setVertexBuffer:_frameConstantsBuffer offset:offsetof(PerFrameConstants, displacementConfig) atIndex:CONFIG_BUFFER_INDEX];
    ...
    [renderCommandEnocder setFragmentBuffer:_frameConstantsBuffer offset:offsetof(PerFrameConstants, displacementConfig) atIndex:1];
    [renderCommandEnocder setFragmentBuffer:_lightsBuffer offset:0 atIndex:0];
    
    [renderCommandEnocder setFragmentTexture:_colorPtexture->GetTexelsTexture() atIndex:IMAGE_TEXTURE_INDEX];
    [renderCommandEnocder setFragmentBuffer:_colorPtexture->GetLayoutBuffer() offset:0 atIndex:IMAGE_BUFFER_INDEX];
    if(_displacementPtexture)
    {
        [renderCommandEnocder setFragmentTexture:_displacementPtexture->GetTexelsTexture() atIndex:DISPLACEMENT_TEXTURE_INDEX];
        [renderCommandEnocder setFragmentBuffer:_displacementPtexture->GetLayoutBuffer() offset:0 atIndex:DISPLACEMENT_BUFFER_INDEX];
        
        [renderCommandEnocder setVertexTexture:_displacementPtexture->GetTexelsTexture() atIndex:DISPLACEMENT_TEXTURE_INDEX];
        [renderCommandEnocder setVertexBuffer:_displacementPtexture->GetLayoutBuffer() offset:0 atIndex:DISPLACEMENT_BUFFER_INDEX];
    }
    ...
    for(int i = 0; i < pav.size(); i++)
    {
        ...
        switch(patchType)
        {
            ...
            case Far::PatchDescriptor::TRIANGLES:
                [renderCommandEnocder drawIndexedPrimitives:MTLPrimitiveTypeLine
...
-(void)_rebuildModel {
    ...
    bool doAdaptive = (_tessellationMode == kTessellationModeMetal),
    doSingleCreasePatch = (_useSingleCrease);
...
#include <metal_stdlib>
        ...
        DEFINE(USE_PTEX_OCCLUSION, _occlusionPtexture != nullptr);
        DEFINE(USE_PTEX_SPECULAR, _specularPtexture != nullptr);
