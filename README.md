...
```
cmake -G Xcode -D NO_MAYA=1 -D NO_PTEX=1 -D NO_DOC=1 \
      -D NO_OMP=1 -D NO_TBB=1 -D NO_CUDA=1 -D NO_OPENCL=1 -D NO_CLEW=1 \
      -D GLFW_LOCATION="*YOUR GLFW INSTALL LOCATION*" \
      ..
make
...
### iOS
  ...
  * Because OpenSubdiv uses a self-built tool (stringify) as part of the build process, first build the stringify target for macOS.
...
```
  ...
  * This will produce an "OpenSubdiv.xcodeproj".
...
mkdir build
cd build
cmake -G "Unix Makefiles" -D NO_MAYA=1 -D NO_PTEX=1 -D NO_DOC=1 -D NO_OMP=1 -D NO_TBB=1 -D NO_CUDA=1 -D NO_CLEW=1 -D NO_OPENCL=0 -D GLFW_LOCATION="/usr/local" -DGLFW_LIBRARY=/usr/local/lib/libglfw3.a ..

make
sudo make install

cd lib
sudo cp -r OpenSubdiv.framework /Library/Frameworks
